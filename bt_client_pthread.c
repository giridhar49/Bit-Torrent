...
int seeder(bt_args_t *bt_args,bt_info_t* my_bt_info)
{
		...
		memset(&handshakebuffer,0,sizeof(tempbuf));
 		...
 		if (n < 0 )
           ...
           // printf("\n Data sent by receiver after handsake is %d",piece);
               ...
               if(ntohl(piece_id)==bt_args->bt_info->num_pieces) close(newsockfd); pthread_exit(NULL); //Exit when the last piece is downloaded
                } // 
		}   
            //  end  of while
